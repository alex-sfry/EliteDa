...
const tradeRouteForm = (validate, loader, removeLoader) => {
  ...
  const tSelect = new TomSelect('#targetSysStation', {
    ...
    options: [{
      'system': $('#targetSysStation').attr('value')
    }],
    loadThrottle: 500,
    hideSelected: true,
    highlight: false,
...
const handleSubmit = function (e) {
  ...
  if (!this.checkValidity()) {
    ...
    $(this).find('input').each(function () {
      this.validity.tooShort && $(this).siblings('.invalid-feedback').text('Name should contain at least 2 characters.');
      this.validity.required && $(this).siblings('.invalid-feedback').text('Field must not be empty.');
    });
